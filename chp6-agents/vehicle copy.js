...
Vehicle.prototype.flee = function(target) {
	var desired = this.location.sub(target);
	desired.normalize();
	desired.multEquals(this.maxspeed);
	desired.subEquals(this.velocity); 	// Reynolds’s formula for steering force
	desired.limit(this.maxforce);
...
Vehicle.prototype.pursue = function(target) {
	var lookAheadTime = this.location.distance(target.location) / this.maxspeed;
	var predictedTarget = target.location.add(target.velocity.mult(lookAheadTime));
